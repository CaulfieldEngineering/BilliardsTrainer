...
class Pipeline:
    ...
    def _recover_blurred(self, frame, calib, frame_shape, detections):
        """Find balls the detector could not see because they were MOVING.

        Joe: "a remedy is mandatory, we can't move forward if we can't get
        accuracy on this 4." Measured on 005048@233: a medium 8ft/s cut
        smears the ball ~17px past its own 28px diameter at the ~1/75s
        shutter this footage was shot at, and — worse than the smear — the
        CONTRAST collapses, because each pixel only holds the ball for part
        of the exposure and averages the rest to felt. The model is trained
        on solid discs, so it returned NOTHING for half a second.

        The pixels still hold the ball. Two things make that recoverable,
        and both matter:

        MEDIAN BACKGROUND, not frame differencing. A frame difference lights
        up everything that moved, and the thing that moved most next to a
        struck cue ball is Joe's cue stick withdrawing through the same
        space — an earlier attempt at this adopted the stick and drifted the
        cue track the wrong way. A median over the last ~1.5s is the static
        scene (felt, rails, chalk marks), so subtracting it isolates what is
        actually there NOW without ranking the stick above the ball.

        RELATIVE colour, not a threshold. Blur washes a ball toward felt, so
        "within X of purple" fails exactly when it is needed. But the ball
        is still closer to ITS OWN measured colour than to any other ball's,
        and every track now carries that reference (mbgr_hist). So the test
        is a contest, not a bar: whichever live track this blob resembles
        most must be the one asking. That rejects the stick and the cue ball
        for free.

        Nothing here names a ball. A recovered blob is handed over as an
        UNNUMBERED detection, so identity still comes from track continuity
        and this can only keep a ball attached, never rename one.
        """
        import os

        import numpy as np
        _dbg = os.environ.get("RECOV_DEBUG")
        def _say(*a):
            if _dbg:
                print("   [recov]", *a, flush=True)
        buf = getattr(self, "_recov_buf", None)
        if buf is None:
            buf = self._recov_buf = __import__("collections").deque(maxlen=15)
        tracks = getattr(self.tracker, "_tracks", [])
        # gate on "its detection just VANISHED" — a struck ball was at REST
        # one frame earlier, so anything keyed on move_streak excludes the
        # only case that matters (measured: it never fired for the cue ball)
        # A struck ball can be gone for the better part of a second before
        # the detector sees it again (measured on 005048@233: 0.5s), so the
        # window has to stay open that long. The colour contest below is
        # what keeps a wide window safe, not a short one.
        lost = [t for t in tracks if t.confirmed and 1 <= t.misses <= 12
                and len(t.mbgr_hist) >= 5]
        buf.append(frame)
        _say(f"called: buf={len(buf)} lost={[(t.id, t.misses, t.committed_number, len(t.mbgr_hist)) for t in lost]}")
        if len(buf) < 6 or not lost:
            return []
        try:
        ...
        gate = max(8.0, self.tracker.max_dist_frac * calib.table.short_side)
        # Detections this track could actually MATCH. A detection its own
        # colour veto has refused is NOT coverage — that was the bug: the
        # veto correctly told the 4's track "the white ball is not yours",
        # and then recovery skipped the search because a detection happened
        # to be nearby. The two have to agree about what counts.
        from .tracking import BallTracker as _BT
        # every OTHER live track's colour, for the contest
        rivals = {o.id: self._track_colour(o) for o in tracks
                  if len(o.mbgr_hist) >= 5}
        out = []
        for t in lost:
            speed = float(np.hypot(t.vx, t.vy))
            px, py = t.x + t.vx, t.y + t.vy
            covered = [(d.x, d.y) for d in detections
                       if not _BT._colour_contradicts(t, d)]
            if any((px - cx) ** 2 + (py - cy) ** 2 <= max(gate, 1.2 * speed) ** 2
                   for cx, cy in covered):
                _say(f"id{t.id}: skipped, detector already covers it")
                continue                      # the detector already has it
            v = hinv @ np.array([px, py, 1.0])
            if abs(v[2]) < 1e-9:
                continue
            sx, sy = float(v[0] / v[2]), float(v[1] / v[2])
            # A ball that vanished FROM REST has no velocity to scale a
            # window by, and its predicted position stays parked at the spot
            # it left — so the gap between prediction and ball GROWS every
            # frame it stays missing. The window has to grow with it: at
            # 005048@233 the 4 was already 298px from its address spot two
            # detect frames in, and a fixed 193px window simply never
            # contained it. Bound by physics per frame elapsed.
            per_frame = max(gate, 1.3 * speed, 0.30 * calib.table.short_side)
            # +2: misses counts frames since the track went UNMATCHED, which
            # lags the moment the ball actually left (it can stay matched to
            # something for a frame or two first). Measured at 005048@233:
            # when misses first read 1 the ball was already 292px away and a
            # 190px window missed it — at exactly the moment it was easiest
            # to find, closest to the seed and still clearly coloured.
            reach_rect = per_frame * (max(1, t.misses) + 2)
            # cap generously: at 005048@233 the ball is already 400px from
            # the spot it left three detect frames in, and a 260px clamp
            # simply never contained it. The colour contest is what keeps a
            # big window safe — it demonstrably rejects felt and the stick.
            win = int(np.clip(reach_rect * (r_raw / max(6.0, self.tracker._ball_r
                                                        or 12.0)), 3 * r_raw, 520))
            x0, y0 = int(max(0, sx - win)), int(max(0, sy - win))
            x1, y1 = int(min(w, sx + win)), int(min(h, sy + win))
            _say(f"id{t.id} misses={t.misses} seed=({sx:.0f},{sy:.0f}) win={win} "
                 f"box=({x0},{y0})-({x1},{y1})")
            if x1 - x0 < 8 or y1 - y0 < 8:
                _say(f"id{t.id}: window degenerate")
                continue
            stack = np.stack([f[y0:y1, x0:x1] for f in buf]).astype(np.float32)
            ...
            mine = self._track_colour(t)
            _say(f"id{t.id}: {len(cnts)} contours, my colour {tuple(int(q) for q in mine)}")
            best, best_d = None, 1e18
            for c in cnts:
                a = cv2.contourArea(c)
                if a < 0.25 * math.pi * r_raw * r_raw:
                    _say(f"   blob area {a:.0f} too small (<{0.25*math.pi*r_raw*r_raw:.0f})")
                    continue
                (_bx, _by), enc_r = cv2.minEnclosingCircle(c)
                # a smeared BALL is at most a few diameters long; a cue
                # stick or a forearm sweeping past is far bigger
                if enc_r > 3.2 * r_raw:
                    _say(f"   blob r={enc_r:.0f} too big (>{3.2*r_raw:.0f}) area={a:.0f}")
                    continue
                m = cv2.moments(c)
                if m["m00"] <= 0:
                    continue
                ...
                rv = min((float(np.linalg.norm(mean - r))
                          for k, r in rivals.items() if k != t.id), default=1e9)
                _say(f"   blob a={a:.0f} r={enc_r:.0f} col={tuple(int(q) for q in mean)} "
                     f"d_mine={d_mine:.0f} best_rival={rv:.0f} "
                     f"{'LOSES' if rv < d_mine else 'WINS'}")
                if rv < d_mine:
                    continue          # it looks more like somebody else's ball
                if d_mine < best_d:
                    best, best_d = (m["m10"] / m["m00"] + x0,
                                    m["m01"] / m["m00"] + y0, mean), d_mine
            _say(f"id{t.id}: " + (f"RECOVERED at ({best[0]:.0f},{best[1]:.0f})"
                                  if best else "nothing accepted"))
            if best is not None:
                rec = Detection(x=float(best[0]), y=float(best[1]),
                                radius=float(r_raw),
                                bgr=tuple(int(q) for q in best[2]),
                                cls=BallClass.UNKNOWN, score=0.30,
                                number=-1)
                # Say WHO this was found for. Recovery has already done the
                # identification — median background, size, and a colour
                # contest against every other ball on the table. Handing it
                # to association as an anonymous blob 300px from the track
                # means the distance gate throws it away, which is exactly
                # what happened on every attempt before this one.
    ...
    @staticmethod
    def _track_colour(t):
        """This track's own measured colour: the per-channel median of its
        recent sampled readings (robust to a glare frame)."""
        import numpy as np
        n = len(t.mbgr_hist)
    ...
    def _apply_detections(self, raw_dets, calib, frame_shape, frame=None):
        ...
        if frame is not None and getattr(self.settings.detection,
                                         "blur_recovery", True):
            try:
                extra = self._recover_blurred(frame, calib, frame_shape,
                                              detections)
                if extra:
